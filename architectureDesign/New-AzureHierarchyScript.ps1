...
foreach ($resourceGroup in $resourceGroups) {
    ...
    foreach ($resource in $resources) {
        ...
        if ($resourceTypeIcons.ContainsKey($resourceLabelType)) {
            ...
            "!includeurl AzurePuml/$($getIcon.Values)" | Out-File -FilePath $outputFile -Append -Encoding utf8
            $resourceLevel = "$($getIcon.Keys)($resourceLabel_, '$resourceLabel', '$resourceLabelType')"
            $resourceLevel.replace("''", """") | Out-File -FilePath $outputFile -Append -Encoding utf8
